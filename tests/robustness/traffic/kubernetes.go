...
var (
	Kubernetes Traffic = kubernetesTraffic{
		...
		readChoices: []random.ChoiceWeight[KubernetesRequestType]{
			{Choice: KubernetesListCurrent, Weight: 20},
			{Choice: KubernetesListStale, Weight: 20},
			{Choice: KubernetesWatchAll, Weight: 60},
		},
		// Please keep the sum of weights equal 100.
...
func (t kubernetesTraffic) RunTrafficLoop(ctx context.Context, c *client.RecordingClient, limiter *rate.Limiter, ids identity.Provider, lm identity.LeaseIDStorage, nonUniqueWriteLimiter ConcurrencyLimiter, keyStore *keyStore, finish <-chan struct{}) {
	...
	g.Go(func() error {
		for {
			...
			err := t.Read(ctx, kc, s, limiter, keyPrefix)
			if err != nil {
				continue
			}
		}
	...
	g.Go(func() error {
		...
		for {
			...
			if lastWriteFailed {
				_, err := t.ListAll(ctx, kc, s, limiter, keyPrefix, t.averageKeyCount, 0)
				if err != nil {
					continue
				}
			}
...
func (t kubernetesTraffic) ListAll(ctx context.Context, kc kubernetes.Interface, s *storage, limiter *rate.Limiter, keyPrefix string, limit int, revision int64) (rev int64, err error) {
	hasMore := true
...
func (t kubernetesTraffic) Read(ctx context.Context, kc kubernetes.Interface, s *storage, limiter *rate.Limiter, keyPrefix string) (err error) {
	op := random.PickRandom(t.readChoices)
	switch op {
	case KubernetesListStale:
		_, rev := s.PickRandom()
		_, err = t.ListAll(ctx, kc, s, limiter, keyPrefix, t.averageKeyCount, rev)
	case KubernetesListCurrent:
		_, err = t.ListAll(ctx, kc, s, limiter, keyPrefix, t.averageKeyCount, 0)
	case KubernetesWatchAll:
		t.Watch(ctx, kc.(kubernetes.Client).Client.KV.(*client.RecordingClient), s, limiter, keyPrefix, 0)
	default:
...
const (
	...
	KubernetesListCurrent KubernetesRequestType = "list_all_current"
	KubernetesListStale   KubernetesRequestType = "list_all_stale"
	KubernetesWatchAll    KubernetesRequestType = "watch_all"
)
