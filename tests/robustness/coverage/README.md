...
2\. Build and run Conformance tests:
...
curl -v -o all_traces.json "http://127.0.0.1:16686/api/traces?limit=0&lookback=2d&service=etcd&service=apiserver"
```
...
```
go test -timeout 60s -run ^TestInterfaceUse$ go.etcd.io/etcd/tests/v3/robustness/coverage
```
